...
import { Users, DoorOpen, LogOut, ClipboardList, Calendar, Loader2, User, Shield } from 'lucide-react';
...
export const Dashboard = ({ availableRooms, setPage }: any) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [statsData, bookingsData, profile] = await Promise.all([
  ...
  return (
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        {(currentProfile?.role === 'Admin' || currentProfile?.role === 'Manager' || currentProfile?.role === 'Staff') && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {(currentProfile?.role === 'Admin' || currentProfile?.role === 'Manager' || currentProfile?.role === 'Receptionist') ? (
          ...
          <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm p-12 text-center space-y-4">
             <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mx-auto text-slate-300">
                <Shield size={32} />
             </div>
             <h3 className="text-lg font-bold text-slate-900">Access Restricted</h3>
             <p className="text-sm text-slate-500 max-w-xs mx-auto">
               You do not have permission to view the live booking feed. Please contact your manager if you believe this is an error.
             </p>
          </div>
        )}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h4 className="font-bold text-slate-900">{currentProfile?.name}</h4>
                <div className="flex items-center gap-1.5">
                  <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest ${
                    currentProfile?.role === 'Admin' ? 'bg-amber-100 text-amber-700' : 
                    currentProfile?.role === 'Manager' ? 'bg-indigo-100 text-indigo-700' : 
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-xs">
                <span className="text-slate-400 font-medium tracking-tight">Access Level</span>
                <span className="text-slate-900 font-bold">{currentProfile?.role === 'Admin' ? 'Superuser' : 'Standard'}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-slate-400 font-medium tracking-tight">Session Time</span>
                <span className="text-slate-900 font-bold">3h 42m</span>
              </div>
            </div>
          </div>
